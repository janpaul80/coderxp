#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# server-setup.sh — One-time server bootstrap (domain-agnostic)
#
# Run as ROOT on the server ONCE after SSL is set up.
#
# What it does:
...
#   DOMAIN=yournewdomain.com /root/server-setup.sh
#   # DOMAIN defaults to the value in .env.local CLIENT_URL if already set
#
# Required env vars:
...
if [[ -z "$DOMAIN" ]]; then
    ...
    echo "Usage: DOMAIN=yournewdomain.com /root/server-setup.sh"
    exit 1
fi
...
section "CoderXP Server Setup — domain: ${DOMAIN} — $(date)"
...
if [[ ! -f "$APP_DIR/apps/server/package.json" ]]; then
    ...
    warn "  DOMAIN=${DOMAIN} ./scripts/deploy.sh"
    warn ""
...
echo ""
echo -e "${GREEN}  ✅ CoderXP server setup complete — ${DOMAIN}${NC}"
echo ""
echo "  Status:"
pm2 list
echo ""
...
echo ""
echo "  Next steps:"
echo "    1. From your LOCAL machine, run: DOMAIN=${DOMAIN} ./scripts/deploy.sh"
echo "       (builds frontend + deploys to /var/www/coderxp/dist)"
echo "    2. Verify: https://${DOMAIN}"
echo ""
